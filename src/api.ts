...
export default () => {
    ...
    router.get("/customer/:custno", async (req, res, next) => {
        ...
        let result = await q.query("select * from customer where custno=$1", custno);
        if (result.rowCount === 0) {
            return next(new HttpError(`Customer with no <${custno}> not found`, 404));
        }
    ...
    router.get("/accounts/:custno", async (req, res, next) => {
        ...
        if (!custno) return next(new HttpError("Missing customer number", 417));
        const q = res.locals.postgres as PostgresLocals;
        let result = await q.query("select * from account where custno=$1", custno);
        res.locals.result = result.rows.map(r => {
            return new Account(`${r.regno}`, `${r.accountno}`, r.amount, moment.utc(r.lastTx));
        });
        next();
    })
    ...
    router.get("/tx/:regno/:accountno", async (req, res, next) => {
        ...
        if (!regno) return next(new HttpError("Missing registration number", 417));
        if (!accountno) return next(new HttpError("Missing account number", 417));
        const q = res.locals.postgres as PostgresLocals;
